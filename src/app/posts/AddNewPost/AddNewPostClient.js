...
export default function AddNewPost() {
  ...
  useEffect(() => {
    const fetchPost = async () => {
      try {
        ...
        setContent(post.newsContent || ""); // ✅ Load content into editor
  ...
  // Sync content with formData.newsContent
  useEffect(() => {
    setFormData((prev) => ({ ...prev, newsContent: content }));
  }, [content]);
  ...
  const handleSubmit = async (e) => {
    ...
    cleanData.append("newsContent", content);
    ...
    if (originalPost) {
      ...
      updateForm.append("newsContent", content);
  ...
  return (
    <div className="form-container">
      <h1 className="addposttitle">
        {resubmitted ? "Change Post" : originalPost ? "Edit Post" : "Add Post"}
      </h1>
